...
class CompositeProvider:
    ...
    def __init__(self, providers: list, max_retries: int = 3,
        ...
        # 熔断状态:(provider_name, capability) -> 解禁时间戳
        self._circuit_until: dict[tuple[str, str], float] = {}

    def _call_with_failover(
        self, method_name: str, *args, include_source: bool = False, **kwargs
    ):
        """按优先级尝试各 provider,主源抛 retriable 错误时重试,最终全部失败抛 ProviderUnavailable。"""
        last_error: Exception | None = None
        for provider in self._providers:
            # 熔断检查
            circuit_key = (provider.name, method_name)
            until = self._circuit_until.get(circuit_key, 0)
            if time.time() < until:
            ...
            for attempt in range(1, self.max_retries + 1):
                try:
                    ...
                    if not e.retriable:
                        ...
                        self._circuit_until[circuit_key] = time.time() + 300
                        break
                    if attempt < self.max_retries:
                        ...
                        self._circuit_until[circuit_key] = time.time() + 60
    ...
    def get_trade_calendar_with_source(
        self, start: date, end: date
    ) -> tuple[list[TradeDay], str]:
        return self._call_with_failover(
            "get_trade_calendar", start, end, include_source=True
        )
