...
def run_backup(db_path: str, backup_dir: str | None = None) -> dict:
    ...
    logger.info("备份完成: %s (%d bytes, %d tables, sha256=%s)", backup_path, size, tables, h)

    # 清理过期备份(保留 30 天)
    _cleanup_old_backups(bdir, settings.trading_backup_retention_days)

    return {"path": str(backup_path), "size": size, "hash": h, "tables": tables}
